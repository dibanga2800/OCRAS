<?php require_once $_SERVER['DOCUMENT_ROOT']. '/OCRAS/core/init.php';
if (!is_logged_in()){
...
if(isset($_GET['add']) || isset($_GET['edit'])){
  $brandQuery1 = $db->query("SELECT * FROM brand ORDER BY brand");
  $parentQuery =$db->query("SELECT * FROM categories WHERE parent = 0 ORDER BY category");
  $title = ((isset($_POST['title']) && $_POST['title'] != '')?sanitize($_POST['title']): '');
  $brand = ((isset($_POST['title']) && !empty($_POST['brand']))?sanitize($_POST['brand']):'');
...
if(isset($_GET['delete_image'])){
  ...
  $dbpath ='/OCRAS/images/products/'.$uploadName;
  if ($mimeType != 'image'){
  ...
  if(isset($_GET['edit'])){
    $insertSql = "UPDATE products SET title = '$title', price = '$price', list_price = '$list_price',
     brand = '$brand', categories = '$category', sizes = '$sizes', image = '$dbpath', description = '$description' WHERE id = '$edit_id ' ";
  }
  $db->query($insertSql);
  header('Location: products.php');
  }
...
<div class="form-group col-md-3">
  ...
  <?php while($b = mysqli_fetch_assoc($brandQuery1)): ?>
  ...
  <div class="form-group col-md-3">
    ...
    <select class="form-control" id="child" name="child" class="form-control">
  </select>
  </div>
...
<div class="form-group col-md-3">
<label for = "Sizes & Qty Preview">Sizes & Qty Preview*:</label>
  ...
  <div class="modal-dialog  modal-lg">
    <div class="modal-content">
      ...
      <div class="modal-footer">
        ...
        <button type="button" class="btn btn-primary pull-right" onclick="updateSizes();jQuery('#sizesModal').modal('toggle');return false;">Save changes</button>
      </div>
    </div>
  </div>
</div>
  ...
  <table class="table table-bordered table-condensed table-stripped">
    ...
    <tbody>
      <?php while($product = mysqli_fetch_assoc($presults)):
        ...
        <tr>
        <td>'
          <a href="products.php?edit=<?=$product['id'];?>" class="btn btn-xs btn-default"><span class="glyphicon glyphicon-pencil"></span>
          <a href="products.php?delete=<?=$product['id'];?>" class="btn btn-xs btn-default"><span class="glyphicon glyphicon-remove-sign"></span>
          </td>
          <td><?=$product['title'];?></td>
          ...
          <td><a href="products.php?featured=<?=(($product['featured'] == 0)?'1':'0');?>&id=<?=$product['id'];?>" class = "btn btn-xs btn-default">
            ...
            </a></a>&nbsp <?=(($product['featured'] == 1)?'Featured Product' :'');?></td></td>
          <td>0</td>
         </tr>
        <?php endwhile; ?>
      </tbody>
